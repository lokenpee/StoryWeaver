...
9) normal：用户输入基本仍在当前节拍轨道内，照着剧本演即可。
10) soft_conflict：用户有偏移，但还能被压回当前节拍边界内。你必须响应用户这回合的动作和意图，但不得把超出当前节拍边界的结果直接写成既成事实；若用户口头快进到下一阶段，只能把它处理成计划、倾向、准备或欲望，并截断到当前节拍内可成立的临时状态，再给下一回合保留自然接回原剧情的轨道。若只是“想跳过一点过程、想更快推进”，优先判 soft_conflict，不要轻易判 hard_conflict。

...
4) 最后再写 direction_script（起点-过程-终点）：你要结合当前节拍原文证据、最近AI输出、最近用户输入，输出可执行框架。direction_script.action_chain 必须是单个字符串，包含3-6段递进动作并用"→"连接；每一段都要写得更具体、更可见，避免“继续推进剧情”“发生互动”这类空泛概括。格式示例：主角放下碗筷起身去玄关→母亲一边收拾桌面一边追问去向→他借着换鞋的动作含糊应声→姐姐从客厅探头打断两人的僵持。
...
7) 当用户输入与原文剧情冲突时：优先保障用户动作与意图的权威性，并可适当参考原文细节；但不必无条件承认其口头宣告的跨时段、跨场景或越界结果已经成立。
7.5) 区分延迟与偏离：用户行为只是把原文事件时间后推（如妈妈让上楼但用户继续看电视）→ 不视为冲突，判 normal；用户行为会逻辑阻断后续剧情前提 → 才进入冲突处理。
8) 冲突分级：conflict_level 只能是 normal / soft_conflict / hard_conflict。
9) normal：用户输入与当前节拍兼容（包括延宕/闲笔/插曲），按用户当前方向推进，尾部留钩子即可。
10) soft_conflict：用户有偏移，但还能被压回当前节拍边界内。你必须响应用户这回合的动作和意图，但不得把超出当前节拍边界的结果直接写成既成事实；若用户口头快进到下一阶段，只能把它处理成计划、倾向、准备或欲望，并截断到当前节拍内可成立的临时状态，再给下一回合保留自然接回原剧情的轨道。若只是”想跳过一点过程、想更快推进”，优先判 soft_conflict，不要轻易判 hard_conflict。尾部钩子方向指向原文轨道自然入口。
...
2) direction_script.action_chain 必须是单行字符串，包含3-6段递进动作并用"→"连接，例如：动作A→动作B→动作C→...→动作N。每一段都必须是具体、可写、可见的小动作或小变化，禁止空泛概括。禁止输出 direction_script.steps 数组。
...
- 执行优先级: 冲突执行要求/冲突处理策略 > 起点/动作链/终点 > 当前节拍原文 > 下一节拍预览。后两者仅作素材参考，不得推翻前两者。

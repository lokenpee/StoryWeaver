...
3) direction_script.action_chain 必须是单个字符串，包含3-6段递进动作并用"→"连接；每段只写一个局部可见动作或即时反应，建议12-28字，禁止复制节拍摘要、原文长句或把多个剧情事件压进一段。格式示例：主角放下碗筷起身去玄关→母亲一边收拾桌面一边追问去向→他借着换鞋的动作含糊应声→姐姐从客厅探头打断僵持。
